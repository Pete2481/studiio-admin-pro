...
export default function CompaniesPage() {
  ...
  // State for dynamic companies data
  const [dynamicCompanies, setDynamicCompanies] = useState<Company[]>([]);

  // Base mock data for development - 3 client companies that match our agents
  const baseMockCompanies: Company[] = [
    {
      id: "mock-1",
      ...
      invoiceEmails: '["invoices@hinterlandproperty.com.au", "accounts@hinterlandproperty.com.au"]',
      password: "temp123",
      logoUrl: undefined,
      isActive: true,
      propertiesCount: 45,
      clientsCount: 12,
      salesVolume: "$2M - $5M",
      permissions: '{"viewCalendar":true,"viewBlankedBookings":true,"viewInvoice":true,"viewService":true}',
    ...
    },
    {
      ...
      email: "anna@driftandstay.com.au",
      invoiceEmails: '["billing@driftandstay.com.au"]',
      password: "temp123",
      logoUrl: undefined,
      isActive: true,
      propertiesCount: 28,
    ...
    },
    {
      ...
      email: "braden@bellelennox.com.au",
      invoiceEmails: '["admin@bellelennox.com.au", "finance@bellelennox.com.au"]',
      password: "temp123",
      logoUrl: undefined,
      isActive: true,
      propertiesCount: 32,
  ...
  // Load dynamic company data from localStorage
  useEffect(() => {
    const loadDynamicCompanies = () => {
      const dynamicCompanies = baseMockCompanies.map(company => {
        try {
          // Load company data from localStorage
          const savedData = localStorage.getItem(`studiio.company.data.${company.id}`);
          const savedLogo = localStorage.getItem(`studiio.company.logo.${company.id}`);
          ...
          if (savedData) {
            ...
            return {
              ...
              type: companyData.company || company.type,
              phone: companyData.phone || company.phone,
              email: companyData.email || company.email,
              invoiceEmails: companyData.invoiceEmails || company.invoiceEmails,
              isActive: companyData.isActive !== undefined ? companyData.isActive : company.isActive,
              logoUrl: savedLogo || company.logoUrl,
              permissions: companyData.permissions ? JSON.stringify(companyData.permissions) : company.permissions,
              sendWelcomeEmail: companyData.sendWelcomeEmail !== undefined ? companyData.sendWelcomeEmail : company.sendWelcomeEmail,
            };
          }
          
          // If no saved data but we have a logo, use it
          if (savedLogo) {
            return {
              ...company,
              logoUrl: savedLogo,
          ...
          console.error(`Failed to load data for company ${company.id}:`, error);
          return company;
        }
      });
      
      setDynamicCompanies(dynamicCompanies);
    };

    loadDynamicCompanies();

    // Listen for localStorage changes to refresh company data
    const handleStorageChange = (e: StorageEvent) => {
      if (e.key && (e.key.startsWith('studiio.company.data.') || e.key.startsWith('studiio.company.logo.'))) {
        loadDynamicCompanies();
      }
    };
  ...
  const totalPages = 1;
  const totalCount = companies.length > 0 ? companies.length : dynamicCompanies.length;

  // Use dynamic companies data (which includes localStorage updates) if no companies are loaded from API
  const displayCompanies = companies.length > 0 ? companies : dynamicCompanies;
                      ...
                      <td className="px-6 py-4">
                        <input type="checkbox" className="rounded border-gray-300" />
                         ...
                         <div className="flex items-center">
                           ...
                           <div className="ml-4">
                             <button
                               onClick={() => router.push(`/t/${currentTenant?.slug}/admin/clients/companies/${company.id}`)}
